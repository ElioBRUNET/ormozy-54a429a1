...
const PricingAndWhy = () => {
  ...
  const features = ['15-minute interval notifications', 'Quick logging interface', 'Daily and weekly summaries', 'Work pattern insights', 'Export your data anytime', 'Available on all devices'];
  const faqData = [{
    id: 1,
  ...
  return <section className="py-12 md:py-20 px-4 relative overflow-hidden bg-muted/30">
      <div className="max-w-7xl mx-auto">
        {/* Unified Section Header */}
        <div className="text-center mb-12 md:mb-16">
          <h2 className="md:text-4xl lg:text-5xl mb-4 font-semibold text-3xl">Ready to get started?</h2>
          <p className="md:text-lg text-muted-foreground max-w-2xl mx-auto font-normal text-sm">
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="flex flex-col">
            <h3 className="text-2xl mb-8 text-center font-semibold">Simple pricing</h3>
            <div className="flex justify-center w-full">
              <PricingCard.Card className="w-full">
                <PricingCard.Header>
                  ...
                  <Button className={cn('w-full font-semibold text-white', 'bg-gradient-to-b from-primary to-primary/90 shadow-[0_10px_25px_rgba(59,130,246,0.3)]')} onClick={() => navigate('/auth')}>
                    Start Free Trial
                  </Button>
                </PricingCard.Header>
                <PricingCard.Body>
                  <PricingCard.List className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                    {features.map(item => <PricingCard.ListItem key={item}>
